...
with open('../client/public/data/'+location+'entry.json', 'w') as outfile:
	...
	for request in range(1):
		...
		r = requests.get('https://www.googleapis.com/customsearch/v1?q=(developer+OR+software)+AND+%22'+location.replace(" ", "+")+'%22&cx=007494023296931513621%3Awwnjyngqucm&exactTerms=junior+OR+grad+OR+graduate+OR+associate&excludeTerms=senior&start='+start+'&key=AIzaSyASVujTnW38PIlTZs9dbbEiGHiGs29Ixlo').json()
